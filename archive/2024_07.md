...
1. [parachain的简化和去重](https://github.com/paritytech/polkadot-sdk/pull/4916)  
    ...
    - 对于build一个节点组件和启动节点的部分，去掉了重复的部分
...
3. [Bridges V2重构回退和pallet_bridge_messages简化](https://github.com/paritytech/polkadot-sdk/pull/4935)  
    ...
    - 为BridgeHubs生成新的weight
    - 对于`polkadot-fellows` bridge进行新的测试

4. [Balance Pallet:在currency实现中增加一些触发事件](https://github.com/paritytech/polkadot-sdk/pull/4936)  
    之前在currency实现中，pallet_balances的实现并没有发出任何`Issued`和`Rescinded`事件的实例。这个PR在impl_currency的适当位置添加了Issued和Rescinded事件，并附上了测试。
...
6. [确定不在`displaced_leaves_after_finalizing`的block gap上引发崩溃](https://github.com/paritytech/polkadot-sdk/pull/4997)  
    崩溃的情况与`warp-sync`有关。在warp sync后，可能会出现区块祖先中存在缺失头部的情况。同时，创世哈希在叶子集合中。在finalizing后的 `displaced_leaves`中，会从已确认的区块向后迭代，直到碰到一个未知的区块，从而导致节点崩溃。  
...
8. [pallet_contracts:修改storage host函数的基准测试，以便在不平衡的存储Trie上运行](https://github.com/paritytech/polkadot-sdk/pull/5036)  
    ...
    - 之前，它们是在一个空的存储Trie上运行的
    - 现在，它们在一个不平衡的存储Trie上运行，以反映最坏情况。
...
9. [在创世中添加注入非认证会话密钥的可能性](https://github.com/paritytech/polkadot-sdk/pull/5078)  
    ...
    - 现在，这个PR允许注入一个新的向量，包含未认证的密钥。

10. [修复:跳过genesis leaf从而让同步顺利进行](https://github.com/paritytech/polkadot-sdk/pull/5103)  
    这个PR主要是解决区块链将创世区块报告为叶子节点的情况。
    问题表现为：区块链在一段时间后卡住，不再导入新的区块。同时还添加了一些额外的调试日志，以便将来更容易识别问题。
...
12. [BEEFY: 关闭finality通知以防止一直保留在通道中](https://github.com/paritytech/polkadot-sdk/pull/5129)  
    ...
    - 之前，最终性通知一直保留在通知通道中，这导致了缓存固定溢出。
    - 现在，创建了一个新任务，定期检查将最终性通知转换为未固定的对应项。
...
3. [网络策略：对速度较慢的peer节点进行回退和禁止，以避免多次提交相同请求](https://github.com/paritytech/polkadot-sdk/pull/5029)  
    ...
    - 以前，我们会向同一个慢速节点提交相同的请求，导致这些慢速节点的声誉被封禁。此外，策略还多次选择同一个慢速节点提交查询，尽管可能存在更好的候选节点。
    - 现在，引入了一个容量为512的LRU DisconnectedPeers，仅跟踪请求进行中的断开连接节点的状态。当DisconnectedPeers检测到一个请求进行中的节点断开连接时，该节点会被暂时拒绝：  
    (1)第一次断开连接：60秒  
    (2)第二次断开连接：120秒  
    (3)第三次断开连接后，该节点被封禁，直到节点存储器衰减其声誉。  
    因此这个PR可以减轻了由于无法及时处理请求而过载的节点的压力。而且，如果一个节点在多次暂时拒绝后仍然被检测为慢速，该节点将被封禁。  
    
    理论上，向同一个节点多次提交相同请求的情况仍然可能发生：   
    (a) 我们暂时拒绝并封禁该节点   
    (b) 网络未发现其他节点——这也可能发生在测试网络中   
    (c) 节点在声誉衰减后重新连接，且仍然响应缓慢

## 文档和资料
1. [Agile-coretime文档](https://wiki.polkadot.network/docs/learn-agile-coretime)
    - Agile-coretime主要是用于为平行链提供区块空间，再也不用为区块链支付过高费用
    - Agile Coretime确保区块空间资源在整个网络中得到最佳分配。

    这个文档页面可以帮助用户更好地了解什么是Agile Coretime

2. [Asynchronous Backing](https://wiki.polkadot.network/docs/learn-async-backing)   
    Backing 是指由一部分验证者或备份小组对平行区块（parablocks）进行验证的过程。这是平行区块验证过程中重要的一步，因为它是确保抗审查性的重要防线。平行区块只需得到一个验证者的支持，因此Backing并不保证平行区块的有效性。   
    这篇文章可以帮助用户了解什么是同步Backing，什么是异步backing以及为什么需要这么做
...
2. [将 `XcmDryRunApi::dry_run_extrinsic`方法改为接收一个`call`参数而不是 `extrinsic`](https://github.com/paritytech/polkadot-sdk/pull/4621)
    接受一个`extrinsic`意味着前端需要先签名以进行`dry-run`，然后再次签名以提交。这会导致糟糕的用户体验，  
    而接受一个`origin`和一个`call`可以解决这个问题，并且还有一个好处，就是可以以任何账户进行`dry-run`，因为它不需要签名。